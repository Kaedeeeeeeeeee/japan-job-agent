...
try {
  const sources = await sql<{ id: string; tenant_key: string; base_url: string }>`SELECT id,tenant_key,base_url
    FROM source_instances WHERE source_kind='hrmos' AND verification_state='verified' ORDER BY tenant_key`.execute(db);
  const selected = sources.rows.filter((row) => requestedTenants.size === 0 || requestedTenants.has(row.tenant_key));
  if (selected.length === 0) throw new Error("No verified HRMOS sources matched");
  const connector = new HrmosConnector();
  const store = createObjectStore();
  for (const row of selected) {
    const source: SourceInstanceRef = { id: row.id, sourceKind: "hrmos", tenantKey: row.tenant_key, baseUrl: row.base_url };
    ...
    const result = await new SourceSyncService(db, connector, store).run({
      ...
      temporalWorkflowId: `manual-hrmos-${row.tenant_key}`,
      temporalRunId: new Date().toISOString(),
    });
    process.stdout.write(`${JSON.stringify({ tenant: row.tenant_key, syncRunId: result.syncRunId,
      kind: result.snapshot?.kind ?? "reused", jobs: result.snapshot?.jobs.length ?? 0,
      newRecords: result.persistedRecords, newVersions: result.persistedVersions })}\n`);
  }
} finally {
  await db.destroy();
}
