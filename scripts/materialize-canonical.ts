...
const sourceKinds = (process.env.MATERIALIZE_SOURCE_KINDS ?? "").split(",").map((value) => value.trim()).filter(Boolean);
const { Pool } = pg;
...
try {
  ...
  process.stdout.write(`${JSON.stringify({ sourceKinds, selected: pending.rows.length,
    versionsCreated: created, versionsReused: reused, mergeRules })}\n`);
} finally {
  await db.destroy();
}
